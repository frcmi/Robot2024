...
public class DriveToPosition extends Command {
    ...
    private static PIDController xPID = new PIDController(
            AutoConstants.kAutoXP,
            AutoConstants.kAutoXI,
            AutoConstants.kAutoXD);
    private static PIDController yPID = new PIDController(
            AutoConstants.kAutoYP,
            AutoConstants.kAutoYI,
            AutoConstants.kAutoYD);
    private static PIDController turnPID = new PIDController(
            AutoConstants.kRotationP,
            AutoConstants.kRotationI,
            AutoConstants.kRotationD);

    private static StructPublisher<Pose2d> positionPublisher = NetworkTableInstance.getDefault()
     ...
     * @param currentPosition a supplier that contains the consitently updated
     *                        position of the bot
     * @param destination     the position that the robot should drive to, relative
     *                        to the field
     */
    ...
    public boolean isFinished() {
        ...
        return Transformations.length(toDestination.getTranslation()) <= SwerveConstants.kAllowedDistanceToDestination
                &&
                Math.abs(toDestination.getRotation()
                        .getRadians()) <= SwerveConstants.kAllowedRotationDifferenceToDestination;
    }
